...
import serial, time, datetime, sys, struct, ctypes
...
class TeensySerial(object):
	...
	def write(self, s):
		if isinstance(s,int):
			# print(f"write called with int: {s}")
			s = bytes(ctypes.c_uint8(s))
		elif isinstance(s,tuple) or isinstance(s,list):
			# TODO: This is probably not correct for Python3, probably need to check each element to see if it's an int or str and do the correct thing
			s = ''.join([chr(c) for c in s]) 

		# print(f"obuf: {type(self.obuf)}  s: {type(s)}")
		if isinstance(s,str):
			s = s.encode('utf-8', errors = 'strict')

		# print(f"obuf: {type(self.obuf)}  s: {type(s)}")
		# print(f"write called with byte: {hexlify(s)}")
		self.obuf += s
		while len(self.obuf) > self.BUFSIZE:
			# self.ser.write(self.obuf[:self.BUFSIZE].encode())
			# print(f"writing bytes: {hexlify(self.obuf[:self.BUFSIZE])}")
			self.ser.write(self.obuf[:self.BUFSIZE])
			self.obuf = self.obuf[self.BUFSIZE:]

	def flush(self):
		if len(self.obuf):
			# self.ser.write(self.obuf.encode())
...
class NANDFlasher(TeensySerial):
	...
	def read_result(self):
		# read status byte
		res = self.readbyte()
		print(f"read_result res: {chr(res)}")
		...
		if (res != 75): #'K'
			if (res == 84): #'T'
				...
				self.close()
				raise NANDError(f"Received unknown error! (Got {hex(res)})")
	...
	def writepage(self, data, pagenr):
		...
		#self.write(0x0)
		#self.write(0x0)
		print(F"writepage pgblock: {pgblock} pgoff: {pgoff} pagenr: {pagenr} pagenr bytes: [{pagenr & 0xFF}, {(pagenr >> 8) & 0xFF}, {(pagenr >> 16) & 0xFF}]")
		self.write(pagenr & 0xFF)
		self.write((pagenr >> 8) & 0xFF)
		self.write((pagenr >> 16) & 0xFF)
	...
	def dump(self, filename, block_offset, nblocks):
		...
		print(f"self.NAND_PAGES_PER_BLOCK: {self.NAND_PAGES_PER_BLOCK}")
		...
		for page in range(block_offset*self.NAND_PAGES_PER_BLOCK, (block_offset+nblocks)*self.NAND_PAGES_PER_BLOCK, 1):
			...
			fo.write(data)
			print("\r%d KB / %d KB"%((page-(block_offset*self.NAND_PAGES_PER_BLOCK)+1)*self.NAND_PAGE_SZ_PLUS_RAS/1024, nblocks*self.NAND_BLOCK_SZ_PLUS_RAS/1024), end=' ')
			sys.stdout.flush()
	...
	def program(self, data, verify, block_offset, nblocks):
		...
		while block < nblocks:
			...
			print("\r%d KB / %d KB"%(((block+1)*self.NAND_BLOCK_SZ_PLUS_RAS)/1024, (nblocks*self.NAND_BLOCK_SZ_PLUS_RAS)/1024), end=' ')
			sys.stdout.flush()
	...
	def testwrite(self):
		self.write(self.CMD_TEST_WRITE)
		if self.read_result() == 0:
			return 0
		return 1

	def debuglog(self):
		self.write(self.CMD_DEBUG_LOG)
...
if __name__ == "__main__":
	...
	if (len(sys.argv) == 3) and (sys.argv[1] == "ps3badblocks"):
		...
		while block < nblocks:
			...
			print("\r%d KB / %d KB"%(((block+1)*(block_plus_ras_sz))//1024, (nblocks*(block_plus_ras_sz))//1024), end=' ')
			sys.stdout.flush()
	...
	elif len(sys.argv) == 4 and sys.argv[3] == "testwrite":
		n.testwrite()
		print()
		print("Performed test write (writes 0xAB to first page), confirm by dumping the first page") 

	elif len(sys.argv) == 4 and sys.argv[3] == "debuglog":
		n.debuglog()
		print()
		print("Told the chip to print something to the UART debug log") 
